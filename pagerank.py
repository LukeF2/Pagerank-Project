...
class WebGraph():
    ...
    def __init__(self, filename, max_nnz=None, filter_ratio=None):
        ...
        n = len(self.url_dict)
        self.P = torch.sparse_coo_tensor(i, v, (n, n), dtype=torch.float)
    ...
    def power_method(self, v=None, x0=None, alpha=0.85, max_iterations=1000, epsilon=1e-6):
        ...
        with torch.no_grad():
            ...
            sum_of_rows = torch.sparse.sum(self.P, 1).indices()
            a = torch.ones([n, 1])
            a[sum_of_rows] = 0
            # main loop
            xprev = x0
            ...
            for i in range(max_iterations):
                ...
                addition = (alpha * x.t() @ a + (1 - alpha)) * v.t()
        ...
        return x1.squeeze()  # Return the final PageRank vector
